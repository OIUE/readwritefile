...
//@SeeAlso({PutFile.class,FetchFile.class, GetFile.class})
...
public class WriteProcessor extends AbstractProcessor {
    ...
    public static final Map<String, Map<String, Object>> WRITEFILE = new HashMap<>();
    ...
    public static final PropertyDescriptor TIME_OUT = new PropertyDescriptor.Builder()
            .name("Last Modified Time TimeOut(millisecond) ")
            .description("Task last modified time timeout.")
            .required(true)
            .defaultValue("600000")
    ...
    public static final Relationship REL_FAILURE = new Relationship.Builder()
            ...
            .build();
    public static final Relationship REL_FRAGMENT_SUCCESS = new Relationship.Builder()
            .name("fragment success")
            .description("Files fragment that have been successfully written to the output directory are transferred to this relationship")
            .build();
    ...
    protected void init(final ProcessorInitializationContext context) {
        ...
        procRels.add(REL_FRAGMENT_SUCCESS);
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        String gfragment_identifier = null;
        final ComponentLog logger = getLogger();
        final StopWatch stopWatch = new StopWatch(true);
        boolean End_o_processing = false;
        final int time_out = context.getProperty(TIME_OUT).asInteger().intValue();
        Path gdotCopyFile = null;
        Path gfinalCopyFile = null;
        final String check_task_time_out = flowFile.getAttribute("check_task_time_out");
        if (check_task_time_out != null) {
            synchronized (WRITEFILE) {
                Iterator<Map.Entry<String, Map<String, Object>>> iterator = WRITEFILE.entrySet().iterator();
            ...
            try {
                ...
                final String fragment_identifier = flowFile.getAttribute(FRAGMENT_ID_ATTRIBUTE);
                final String fragment_index = flowFile.getAttribute(FRAGMENT_INDEX_ATTRIBUTE);
                final String fragment_count = flowFile.getAttribute(FRAGMENT_COUNT_ATTRIBUTE);
                Long fragment_countl = 0L;
                ...
                if (fragment_index != null) {
                    session.exportTo(flowFile, dotCopyFile, true);
                }

                synchronized (WRITEFILE) {
                    Map<String, Object> fileinfo;
                    if (WRITEFILE.containsKey(fragment_identifier)) {
                        fileinfo = WRITEFILE.get(fragment_identifier);
                    } else {
                        fileinfo = new ConcurrentHashMap<>();
                        fileinfo.put("count", 0L);
                        fileinfo.put("finalCopyFile", finalCopyFile);
                        fileinfo.put("dotCopyFile", dotCopyFile);
                        fileinfo.put("fragment_identifier", fragment_identifier);
                        WRITEFILE.put(fragment_identifier, fileinfo);
                    }

                    Long count = (Long) fileinfo.get("count") + ((fragment_index != null) ? 1 : 0);
                    fileinfo.put("count", count);

                    if (fileinfo.containsKey(FRAGMENT_COUNT_ATTRIBUTE) && fragment_countl == 0) {
                        fragment_countl = (Long) fileinfo.get(FRAGMENT_COUNT_ATTRIBUTE);
                    }
                    getLogger().debug("tempCopyFile:{} \ncopyFile:{} \nfilename:{} \nfinalCopyFile:{}\n fragment_countl:{} count:{} \n fragment_countl==count:{}  \n line:{}", new Object[]{tempCopyFile, copyFile, filename, finalCopyFile, fragment_countl, count, flowFile});
                    if (fragment_countl.longValue() == count.longValue()) {
                        End_o_processing = true;
                        WRITEFILE.remove(fragment_identifier);
                        gdotCopyFile = dotCopyFile;
                        gfinalCopyFile = finalCopyFile;
                    } else {
                        fileinfo.put(FRAGMENT_COUNT_ATTRIBUTE, fragment_countl);
                        session.transfer(flowFile, REL_FRAGMENT_SUCCESS);

                        fileinfo.put("lasttime", System.currentTimeMillis());
                        WRITEFILE.put(fragment_identifier, fileinfo);
                        return;
                    }
                }
        ...
        for (int i = 0; i < 10; i++) {
            if (gdotCopyFile.toFile().renameTo(gfinalCopyFile.toFile())) {
                renamed = true;
        ...
        if (!renamed) {
            if (Files.exists(gdotCopyFile) && gdotCopyFile.toFile().delete()) {
                logger.debug("Deleted dot copy file {}", new Object[]{gdotCopyFile});
            }

            Map<String, Object> fileinfo = new ConcurrentHashMap<>();
            fileinfo.put("finalCopyFile", gfinalCopyFile);
            fileinfo.put("dotCopyFile", gdotCopyFile);
            fileinfo.put("lasttime", System.currentTimeMillis());
            fileinfo.put("fragment_identifier", gfragment_identifier);
            WRITEFILE.put(gfragment_identifier, fileinfo);
            throw new ProcessException("Could not rename: " + gdotCopyFile);
        } else {
            logger.info("Produced copy of {} at location {}", new Object[]{flowFile, gfinalCopyFile});
        }

        session.getProvenanceReporter().send(flowFile, gfinalCopyFile.toFile().toURI().toString(), stopWatch.getElapsed(TimeUnit.MILLISECONDS));
        session.transfer(flowFile, REL_SUCCESS);
    }
